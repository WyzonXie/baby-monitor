...
| `SPEECH_THRESHOLD` | `0.5` | 超过此分数认为有大人在说话，计数清零 |
...
**5. 告警的条件为什么不是"宝宝在哭"，而是"宝宝在哭且没人管"？**
宝宝翻身哼两声、放下时闹一下、醒了叫两声大人马上抱起来——这些每天都在发生。按"哭了就报"，一天几十条，人很快就不看了，**等真出事那条也一起被忽略**。报警的价值不在"宝宝哭了"（常态），在"哭了很久而且没人管"（事故）。
判断"有没有人管"用的是同一个模型的 `Speech` 标签，几乎零额外开销。规则是**说话优先**：这一秒只要判定有大人在说话，就把哭声计数清零、告警标记复位——因为这次哭已经被人接手了，下一次再哭算新的一次事件。

**6. "有没有人在旁边"为什么用声音判断，不用摄像头画面？**
更直接的做法是分析画面里有没有人，但没有采用，三个理由：
① 我们要判断的不是"有没有人"，是"**有没有人在管**"——大人在旁边睡着了、刷手机，画面里都有人，但没人在管；说话是"正在互动"的证据，看见人只是"人在附近"的证据。
② 成本差一个数量级——"有没有人说话"是白送的（模型本来就把 500 多类声音都算了，只是之前没去看这一类），而视觉方案要解视频、逐帧跑人体检测、再和音频对齐，CPU 从"感觉不到"变成"风扇转起来"。
③ 摄像头是对着婴儿床拍的，不是对着房间拍的——大人站在床边、坐在椅子上、在门口，很可能都在画面外，**画中没人 ≠ 房里没人**，而这个误判方向恰恰最危险。
代价也如实记着：大人一声不吭地抱着拍睡，声音方案会误报。等实际跑一段时间，日志会说明误报到底出在哪儿，**到那时有数据再决定要不要上视觉**。

**7. 为什么按名字找 `Baby cry, infant cry`，而不是取模型给的第一名？**
...
- **参数写死在代码里**，调参需要改源码。
- **大人不出声地哄睡会误报**——见设计决策 6，属于已知代价。
- 只做了音频，摄像头的画面完全没用上。
- `cry_log.csv` 前后两段列数不同（新增了说话分数一列），分析历史数据时要注意。
